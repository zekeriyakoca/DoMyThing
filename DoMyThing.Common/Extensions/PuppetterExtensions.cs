...
namespace PuppeteerSharp
{
    public static class PuppeteerExtentions
    {
        ...
        {
            var input = await page.QuerySelectorWhenAppearAsync(selector, 20000);
            await input.FocusAsync();

            foreach (var ch in text.ToString())
            {
